...
--                 D1,D2 + D10  is not matching the condition but is in the LEFT Table column (dept_id)
...
ON d.dept_id=e.dept_id;
 ...
 -- IN RIGHT JON RIGHT TABLE GETS THE PRIORITY
...
-- And aslo be aware of the fact that which table is to be considered as the main table (here employee)
...
-- The result of the statement from the 'FROM' clause till the  Left of the the 'LEFT JOIN project p' 
-- is considered entirely as the LEFT Table ( Result table) and the RIGHT Table is the project Table.
-- Its no the manager table to be only considered as the LEFT Table here.
-- A cumulatiove result table from the 'FROM' till that 'LEFT JOIN' is to be considered as the LEFT Table.
...
--  Oracle PostgreSQL and SQL Server do support the full-outer join.
